...
class InvestorController extends GetxController {
  ...
  void onInit() {
    ...
    getAspireList(
        ...
        isRefresh: false);
    print(
        "exhibitor type ${authenticationManager.getExhibitorType()?.toLowerCase()}");
    super.onInit();
  }
  ...
  Future<void> getAspireList({required requestBody, required isRefresh}) async {
    ...
    StartNetworkingModel? model = await apiService.getAspireSearch(requestBody);
  ...
  Future<void> _loadMore() async {
    scrollController.addListener(() async {
      if (hasNextPage == true &&
        ...
        try {
          StartNetworkingModel? model =
              await apiService.getAspireSearch(newRequestBody);
          if (model.status! && model.code == 200) {
  ...
  Future<void> getUserDetail(String id) async {
    ...
    isLoading(true);
    StartNetworkingModel? model =
        await apiService.getAspireDetails(requestBody);
    isLoading(false);
    if (model.status! && model.code == 200) {
  ...
  Future<void> bookAppointment(
    ...
    isLoading(true);
    BookmarkCommonModel? model = await apiService.bookAspireAppointment(body);
    isLoading(false);
