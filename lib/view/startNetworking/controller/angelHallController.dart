...
class AngelHallController extends GetxController {
  ...
  Future<void> getAngelAllyHallList(
    ...
    AngelAllyModel? model = await apiService.getAngelAllyHallList(
        requestBody, AppUrl.searchAspireList);
    isFirstLoading(false);
  ...
  Future<void> getAngelAllyLoadMore() async {
    scrollController.addListener(() async {
      if (hasNextPage == true &&
        ...
        try {
          AngelAllyModel? model = await apiService.getAngelAllyHallList(
              newRequestBody, AppUrl.searchAspireList);
          if (model.status! && model.code == 200) {
  ...
  Future<void> getAngelHallDetail(sessionId) async {
    ...
    AngelAllyModel? model = await apiService.getAngelAllyDetail(
        url: AppUrl.searchAspireList, payload: payloadBody);
    loading(false);
    if (model.status! && model.code == 200) {
      _angelBody(model.body?.sessions?[0]);
  ...
  showScheduleDialog(BuildContext context) async {
    ...
    var newSlotData = filterMeetingSlotDate(
        authManager.getTimezone() ?? "", timeslotsList);
    timeslotsList.clear();
  ...
  Future<void> startNetworkingSlotList({required requestBody}) async {
    loading(true);
    StartNetworkingSlot? model =
    await apiService.startNetworkingSlot(requestBody);
    loading(false);
    if (model.status! && model.code == 200) {
  ...
  Future<void> bookAppointment(
    ...
    await apiService.bookAspireAppointment(body);
    loading(false);
