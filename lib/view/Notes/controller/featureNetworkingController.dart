...
class FeatureNetworkingController extends GetxController {
  ...
  Future<void> getAttendeeList(
    ...
    RepresentativeModel? model = await apiService.getUserList(
        requestBody, "${AppUrl.usersListApi}/search");
    ...
    itemPositionsListener.itemPositions.addListener(() async {
      ...
      if (positions.isNotEmpty) {
        ...
        if (hasNextPage == true &&
          ...
          try {
            RepresentativeModel? model = await apiService.getUserList(
                newRequestBody, "${AppUrl.usersListApi}/search");
            if (model.status! && model.code == 200) {
              hasNextPage = model.body!.hasNextPage!;
              _pageNumber = _pageNumber + 1;
              attendeeList.addAll(model.body!.representatives!);
              userIdsList.clear();
              if(model.body!.representatives !=null && model.body!.representatives!.isNotEmpty){
                userIdsList.addAll(
  ...
  Future<void> getBookmarkIds() async {
    try {
      ...
      BookmarkIdsModel? model = await apiService
          .getBookmarkIds({"items": userIdsList,"item_type": MyConstant.attendee});
      if (model.status! && model.code == 200) {
        if (model.body != null) {
