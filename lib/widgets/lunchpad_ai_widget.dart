...
import '../view/schedule/view/session_details_page.dart';
...
class LaunchpadAIWidget extends GetView<ForYouController> {
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                        ...
                        CustomTextView(
                            text: data.startDatetime != null
                                ? UiHelper.displayDateFormat(
                                    ...
                                    timezone:
                                        controller.authManager.getTimezone())
                                : "",
                            maxLines: 1,
                            color: colorSecondary,
                            fontWeight: FontWeight.w600,
                    ...
                    CustomTextView(
                        text:
                            "${UiHelper.displayTimeFormat(date: data.startDatetime ?? "", timezone: controller.authManager.getTimezone())}"
                            " - ${UiHelper.displayTimeFormat(date: data.endDatetime ?? "", timezone: controller.authManager.getTimezone())}",
                        color: colorSecondary,
  ...
  openSessionDetail(id) async {
    if (Get.isRegistered<SessionController>()) {
      ...
      controller.isLoading(true);
      var apiResult = await sessionController.getSessionDetail(requestBody: {"id": id});
      controller.isLoading(false);
      if (apiResult["status"]) {
